...
short int paused = 0;
// this tells us whether or not we want to draw lines between each charge or not
bool drawLinesBetweenCharges = true;
...
int main( int argc, char* args[] )
{
	...
	// open the help file
    ShellExecute(NULL, "open", "ReadMe.txt", NULL, "", SW_SHOWNORMAL);
    ...
    setup_charges(c_orbit);
    
    //OpenFile("ReadMe.txt",NULL,NULL);
    	...
    	while( SDL_PollEvent( &event ) )
    	{
			...
			else if( event.type == SDL_KEYDOWN )
			{
				...
				else if(k == SDLK_RETURN)
				{
					...
					drawLinesBetweenCharges ^= 1; // toggle lines
				}
